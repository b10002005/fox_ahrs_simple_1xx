...
void MahonyAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) {
	...
	qb = q1_m;
	qc = q2_m;
	q0_m += (-qb * gx - qc * gy - q3_m * gz);
	q1_m += (qa * gx + qc * gz - q3_m * gy);
	q2_m += (qa * gy - qb * gz + q3_m * gx);
	q3_m += (qa * gz + qb * gy - qc * gx); 
	
	// Normalise quaternion
	recipNorm = invSqrt1(q0_m * q0_m + q1_m * q1_m + q2_m * q2_m + q3_m * q3_m);
	q0_m *= recipNorm;
	q1_m *= recipNorm;
	q2_m *= recipNorm;
	q3_m *= recipNorm;
...
void MahonyAHRSupdateIMU(float gx, float gy, float gz, float ax, float ay, float az) {
	...
	if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
		...
		halfvx = q1_m * q3_m - q0_m * q2_m;
		halfvy = q0_m * q1_m + q2_m * q3_m;
		halfvz = q0_m * q0_m - 0.5f + q3_m * q3_m;
